...
const latestContents = Array.from({length: 21}, (_, i) => ({
  title: `최신작 ${i+1}`,
...
function renderPopularContents() {
  ...
  popularContents.forEach((content, idx) => {
    const card = document.createElement('div');
    card.className = 'content-card content-popular-card';
    let rankHtml = '';
    if (content.rank === 1) {
...
function renderLatestContents() {
  ...
  if (!grid) return;
  grid.innerHTML = '';
  latestContents.forEach((content, idx) => {
    const card = document.createElement('div');
    ...
    card.innerHTML = `
      ...
      <div class="content-info">
        ...
        <div class="content-meta">
          ...
          <span class="content-value">${content.rating}</span>
          ...
          <span class="content-value">${content.views}</span>
        </div>
      </div>
    `;
...
function renderFavoriteContents() {
  ...
  if (!grid) return;
  grid.innerHTML = '';
  favoriteContents.forEach((content, idx) => {
    const card = document.createElement('div');
